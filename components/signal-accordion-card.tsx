...
import { formatCanonicalValue, formatTrendPct, formatTrendAbsolute, inferValueType } from "@/lib/signal-format"
import type { SignalWithData } from "@/lib/signals-service"
import type { SignalInterpretation } from "@/lib/interpretation-service"

/** Which trend period to show in the ticker (or both). */
export type TrendPeriod = "30d" | "90d"

/** How to display trend in the ticker: percentage change or absolute delta. */
export type TrendDisplay = "percent" | "absolute"
...
interface SignalAccordionCardProps {
  ...
  /** Ticker: which trend period (30d or 90d). */
  trendPeriod?: TrendPeriod
  /** Ticker: show trend as % or absolute change. */
...
export function SignalAccordionCard({ 
  ...
  /** Format upload filename for display, e.g. "zoho-crm-deals.csv" → "zoho - crm - deals" */
  const formatSourceFileName = (fileName: string) =>
    (fileName || "")
      .replace(/\.[^.]+$/, "")
      .replace(/[-_]+/g, " - ")
      .trim() || "File upload"
  ...
  const getCalculationExplanation = () => {
    ...
    const source = signal.source_type || "upload"
    const sourceLabel =
      source === "upload" && signal.source_metadata?.fileName
        ? formatSourceFileName(signal.source_metadata.fileName)
        : source === "api"
          ? "API integration"
          : source === "upload"
            ? "File upload"
  ...
  const canonicalValue = formatCanonicalValue(signal)
  const trendPct = trendPeriod === "30d" ? signal.trend_30d_pct : signal.trend_90d_pct
  const trendDelta = trendPeriod === "30d" ? signal.trend_30d_delta : signal.trend_90d_delta
  const trendStr =
    trendDisplay === "absolute"
      ? formatTrendAbsolute(signal, trendDelta)
      : formatTrendPct(trendPct)
  const hasTrend = trendDisplay === "percent" ? trendPct != null : trendDelta != null

  // Deterministic variant index from signal id so the same signal always gets the same phrasing
  const variantIndex =
    typeof signal.id === "string"
      ? signal.id.split("").reduce((acc, c) => acc + c.charCodeAt(0), 0)
      : 0

  // Last period = most recent (e.g. January); previous period = prior (e.g. December)
  const lastPeriod =
    trendPeriod === "30d"
      ? signal.trend_30d_to_label ?? "the latest month"
      : signal.trend_90d_to_label ?? "the last 3 months"
  const prevPeriod =
    trendPeriod === "30d"
      ? signal.trend_30d_from_label ?? "the previous month"
      : signal.trend_90d_from_label ?? "the prior 3 months"

  const valueType = inferValueType(signal)
  const isTime = valueType === "time"

  // For sentences use magnitude (e.g. "86.7%" not "-86.7%")
  const trendStrForSentence = trendStr.replace(/^\-/, "").replace(/^\+/, "")

  // Format: "[Signal] value for [last period] is $X, [increased/decreased] [X%] compared to [prev period]." with variations
  const execSummary =
    hasTrend && trendStr !== "—"
      ? (() => {
          const dir = trendDisplay === "percent" ? (trendPct ?? 0) : (trendDelta ?? 0)
          const direction = dir > 0 ? "up" : dir < 0 ? "down" : "unchanged"

          if (direction === "unchanged") {
            const unchangedPhrases = [
              `${signal.name} value for ${lastPeriod} is ${canonicalValue}, unchanged compared to ${prevPeriod}.`,
              `${signal.name} for ${lastPeriod} is ${canonicalValue}, flat vs ${prevPeriod}.`,
              `In ${lastPeriod}, ${signal.name} holds steady at ${canonicalValue} (same as ${prevPeriod}).`,
            ]
            return unchangedPhrases[variantIndex % unchangedPhrases.length]
          }

          if (direction === "down") {
            const downPhrases = [
              `${signal.name} value for ${lastPeriod} is ${canonicalValue}, down ${trendStrForSentence} compared to ${prevPeriod}.`,
              `${signal.name} for ${lastPeriod} is ${canonicalValue}, a ${trendStrForSentence} decrease compared to ${prevPeriod}.`,
              `In ${lastPeriod}, ${signal.name} is ${canonicalValue}, ${trendStrForSentence} lower than ${prevPeriod}.`,
            ]
            return downPhrases[variantIndex % downPhrases.length]
          }

          // up
          const upPhrases = [
            `${signal.name} value for ${lastPeriod} is ${canonicalValue}, up ${trendStrForSentence} compared to ${prevPeriod}.`,
            `${signal.name} for ${lastPeriod} is ${canonicalValue}, a ${trendStrForSentence} increase compared to ${prevPeriod}.`,
            `In ${lastPeriod}, ${signal.name} is ${canonicalValue}, ${trendStrForSentence} higher than ${prevPeriod}.`,
          ]
          return upPhrases[variantIndex % upPhrases.length]
        })()
      : `${signal.name} value for ${lastPeriod} is ${canonicalValue}. No change data compared to ${prevPeriod}.`

  const handleShare = () => {
    const trendLabel = trendPeriod === "30d" ? "30d" : "90d"
    const trendSuffix =
      hasTrend && trendStr !== "—" ? ` (${trendStr} ${trendLabel})` : ""
    if (navigator.share) {
      navigator.share({
        title: signal.name,
        text: `${signal.name}: ${canonicalValue}${trendSuffix}`,
      })
    } else {
      navigator.clipboard.writeText(`${signal.name}: ${canonicalValue}${trendSuffix}`)
    }
  }
  ...
  return (
    <Card className="overflow-hidden border border-border bg-card hover:shadow-md transition-shadow">
      {/* L1: Main line — Signal name | Absolute value | Direction | Change amount */}
      <div className="p-4">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <h3 className="font-semibold text-foreground text-base leading-tight truncate min-w-0 flex-1">
            {signal.name}
          </h3>
          <div className="flex items-center gap-2 shrink-0 tabular-nums">
            <span className="text-lg font-bold text-foreground">{canonicalValue}</span>
            {hasTrend && trendStr !== "—" ? (
              <>
                <span
                  className={cn(
                    "flex items-center",
                    trendDisplay === "percent" && (trendPct ?? 0) > 0 && "text-emerald-600",
                    trendDisplay === "percent" && (trendPct ?? 0) < 0 && "text-red-500",
                    trendDisplay === "absolute" && (trendDelta ?? 0) > 0 && "text-emerald-600",
                    trendDisplay === "absolute" && (trendDelta ?? 0) < 0 && "text-red-500",
                    (trendDisplay === "percent" && (trendPct ?? 0) === 0) || (trendDisplay === "absolute" && (trendDelta ?? 0) === 0) ? "text-muted-foreground" : ""
                  )}
                  aria-hidden
                >
                  {(trendDisplay === "percent" ? (trendPct ?? 0) : (trendDelta ?? 0)) > 0 && <ArrowUp className="h-4 w-4" />}
                  {(trendDisplay === "percent" ? (trendPct ?? 0) : (trendDelta ?? 0)) < 0 && <ArrowDown className="h-4 w-4" />}
                  {(trendDisplay === "percent" ? (trendPct ?? 0) : (trendDelta ?? 0)) === 0 && <Minus className="h-4 w-4" />}
                </span>
                <span
                  className={cn(
        ...
        {signal.category && (
          <Badge variant="secondary" className="text-[10px] uppercase tracking-wide font-medium mt-1.5 shrink-0">
            {signal.category}
          </Badge>
        )}
        ...
        <div className="flex items-center justify-between mt-3">
          ...
          {/* Source label: show upload filename when available, e.g. "zoho - crm - deals" */}
          <span className="text-[10px] text-muted-foreground">
            {signal.source_type === "upload" && signal.source_metadata?.fileName
              ? formatSourceFileName(signal.source_metadata.fileName)
              : signal.source_type === "api"
                ? "API"
              ...
              <div className="space-y-4 pt-2">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="p-3 rounded-lg bg-background border border-border">
                    ...
                    <div className="font-semibold text-foreground">{calc.sourceLabel}</div>
                    <p className="text-xs text-muted-foreground">
                      {calc.source === "upload" ? "From your uploaded file" : calc.source === "api" ? "API integration" : "Connected source"}
                    </p>
                  </div>
                </div>
