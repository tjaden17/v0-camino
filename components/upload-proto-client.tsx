...
import { applyTemplateToColumns, templateMatchesColumns, type ColumnMappingTemplateShape } from "@/lib/column-mapping-utils"
import {
  Dialog,
  DialogContent,
  DialogFooter,
...
export interface TabData {
  ...
  name: string
  fileName: string
  /** XLSX sheet name for multipart matching */
  sheetName?: string
  columns: string[]
...
export interface TabAnswers {
  ...
  /** When rowType is "other", used for signal names (e.g. "Shifts per Month"). */
  customRowLabel?: string | null
  /** Columns to add "X by [column]" breakdown signals (e.g. "Shifts by Pharmacy Name"). */
...
interface ParsedFile {
  fileName: string
  tabs: TabData[]
  /** Kept for large uploads: send as multipart to avoid body size limit */
  file?: File
}
...
/** Capitalize first letter, rest lowercase (e.g. "shifts" → "Shifts"). */
...
export function generateSignals(tab: TabData, answers: TabAnswers): GeneratedSignal[] {
  ...
  const { rowType, metricColumn, dateColumn, customRowLabel } = answers
  if (!rowType) return signals

  const label =
    rowType === "other" && customRowLabel?.trim()
      ? capitalizeLabel(customRowLabel)
      : (ROW_TYPE_OPTIONS.find(o => o.value === rowType)?.label || rowType)
  ...
  // 5. Group-by signals: (a) auto from whitelist, (b) user-selected columns
  const textCols = tab.columns.filter(c =>
    tab.columnTypes[c] === "text" && c.toLowerCase() !== "id" && c.toLowerCase() !== "email"
  )
  const groupByAdded = new Set<string>()

  const addGroupBySignal = (col: string) => {
    if (groupByAdded.has(col)) return
    const values = tab.sampleRows.map(r => (r[col] || "").trim()).filter(Boolean)
    const uniques = new Set(values)
    if (uniques.size < 2 || uniques.size > 100) return
    groupByAdded.add(col)
    signals.push({
      name: `${label} by ${col}`,
  ...
  for (const col of textCols) {
    ...
    if (isGroupable) addGroupBySignal(col)
  }

  for (const col of answers.groupByColumns ?? []) {
    if (textCols.includes(col)) addGroupBySignal(col)
  }

  // 6. Metric-by-dimension signals (scalable: Total/Average {metric} by {col})
  if (metricColumn && metricColumn !== "none") {
    const numericValues = tab.sampleRows
      .map(r => {
        const cleaned = (r[metricColumn] || "")
          .replace(/^[A-Z]{3}\s*/i, "")
          .replace(/[$€£¥,\s%]/g, "")
    ...
    if (numericValues.length > 0) {
      for (const col of groupByAdded) {
        signals.push({
          name: `Total ${metricColumn} by ${col}`,
          description: `Sum of ${metricColumn} grouped by ${col}`,
        ...
        signals.push({
          name: `Average ${metricColumn} by ${col}`,
          description: `Average ${metricColumn} per group by ${col}`,
          operation: "group_by_avg",
          valueColumn: metricColumn,
          dateColumn,
          groupByColumn: col,
          tabKey: tab.key,
      ...
      {isActive && (
        <CardContent className="px-4 pb-4 pt-0 border-t border-border">
          ...
          <div className="space-y-4">
                    ...
                    <button
                      ...
                      onClick={() => {
                        onUpdateAnswers({
                          ...
                          customRowLabel: opt.value === "other" ? (answers.customRowLabel ?? null) : null,
                        })
                      }}
                      className={cn(
              ...
              {currentType === "other" && (
                <div className="mt-3">
                  <label className="text-sm font-medium text-foreground mb-1.5 block">
                    What do you call these rows?
                  </label>
                  <Input
                    type="text"
                    placeholder="e.g. shifts, bookings, orders"
                    value={answers.customRowLabel ?? ""}
                    onChange={(e) => onUpdateAnswers({ customRowLabel: e.target.value || null })}
                    className="h-9 text-sm"
                  />
                  <p className="text-[11px] text-muted-foreground mt-1">
                    Used in signal names (e.g. &quot;Shifts per Month&quot;, &quot;Total Shifts&quot;).
                  </p>
                </div>
              )}
            </div>
            ...
            {/* Q4: Break down by (optional) */}
            {currentType && (() => {
              const textColsForGroup = tab.columns.filter(c =>
                tab.columnTypes[c] === "text" && c.toLowerCase() !== "id" && c.toLowerCase() !== "email"
              )
              const groupByCandidates = textColsForGroup.filter(col => {
                const values = tab.sampleRows.map(r => (r[col] || "").trim()).filter(Boolean)
                const uniques = new Set(values)
                return uniques.size >= 2 && uniques.size <= 100
              })
              const selected = new Set(answers.groupByColumns ?? [])
              if (groupByCandidates.length === 0) return null
              return (
                <div>
                  <label className="text-sm font-medium text-foreground mb-2 block">
                    Also break down by (optional)
                  </label>
                  <p className="text-[11px] text-muted-foreground mb-2">
                    Get a signal per group (e.g. &quot;Shifts by Pharmacy Name&quot;).
                  </p>
                      ...
                      >
                        <Checkbox
                          ...
                          onCheckedChange={(checked) => {
                            const next = checked
                              ? [...(answers.groupByColumns ?? []), col]
                              : (answers.groupByColumns ?? []).filter(c => c !== col)
                            onUpdateAnswers({ groupByColumns: next })
                          }}
                        />
                        {col}
                      </label>
...
export function UploadProtoClient({ organizationId }: { organizationId?: string | null } = {}) {
  ...
  const [generateResult, setGenerateResult] = useState<{
    ...
    count: number
    errors?: string[]
    ticketFilterSummary?: { tabName: string; ticketsFiltered: number; ticketsTotal: number; filterReasons: { reason: string; count: number }[] }[]
  } | null>(null)
  ...
  const fetchTemplates = useCallback(() => {
    const url = organizationId
      ? `/api/upload/templates?organizationId=${encodeURIComponent(organizationId)}`
  ...
  // When tabs or templates load: for tabs with empty answers, apply matching template or inference
  useEffect(() => {
    if (allTabs.length === 0) return
    let applied = false
    const updates: Record<string, TabAnswers> = {}
    for (const tab of allTabs) {
      const current = tabAnswers[tab.key]
      if (current?.rowType != null) continue
      const empty = !current || (current.rowType === null && current.metricColumn === null && current.dateColumn === null)
      if (!empty) continue
      const matching = mappingTemplates.find(t => templateMatchesColumns(t, tab.columns))
      if (matching) {
        const result = applyTemplateToColumns(matching, tab.columns)
        updates[tab.key] = {
          rowType: (result.rowType as RowType) ?? null,
          metricColumn: result.metricColumn ?? null,
          dateColumn: result.dateColumn ?? null,
          customRowLabel: result.customRowLabel ?? null,
    ...
    if (applied) {
      setTabAnswers(prev => ({ ...prev, ...updates }))
    }
  }, [allTabs.map(t => t.key).join(","), mappingTemplates.length])
  ...
  const handleFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      setParsedFiles(prev => [...prev, { fileName: file.name, tabs, file }])

      // Start with empty answers so matching template or inference can be applied in effect
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const useMultipart = payload.length > 4_000_000 && parsedFiles.every(pf => pf.file != null)
      // #region agent log
      try {
        fetch('http://127.0.0.1:7242/ingest/bc0a0876-b22a-43a2-8bb5-3b0f14e7c9c0',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'upload-proto-client.tsx:handleGenerate',message:'payload size before fetch',data:{payloadLength:payload.length,tabCount:tabPayloads.length,signalCount:signals.length,useMultipart},timestamp:Date.now(),hypothesisId:'H3'})}).catch(()=>{})
      } catch (_) {}
      // #endregion
      ...
      if (useMultipart) {
        ...
        const tabPayloadsMeta = []
        for (let fileIndex = 0; fileIndex < parsedFiles.length; fileIndex++) {
          const pf = parsedFiles[fileIndex]
          for (const tab of pf.tabs) {
            if (!completedTabs.some(ct => ct.key === tab.key)) continue
            tabPayloadsMeta.push({
              tabKey: tab.key,
              tabName: tab.name,
        ...
        formData.append("metadata", JSON.stringify({
          tabs: tabPayloadsMeta,
          signals,
          ...(organizationId ? { organizationId } : {}),
        }))
        for (let i = 0; i < parsedFiles.length; i++) {
          formData.append(`file_${i}`, parsedFiles[i].file!)
        }
        res = await fetch("/api/upload/generate", { method: "POST", body: formData })
      } else {
        res = await fetch("/api/upload/generate", {
          method: "POST",
      ...
      // #region agent log
      try {
        fetch('http://127.0.0.1:7242/ingest/bc0a0876-b22a-43a2-8bb5-3b0f14e7c9c0',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'upload-proto-client.tsx:afterFetch',message:'response received',data:{status:res.status,responseLength:text.length,responsePreview:text.slice(0,120)},timestamp:Date.now(),hypothesisId:'H2'})}).catch(()=>{})
      } catch (_) {}
      // #endregion
      let data: { error?: string; details?: string; signalsCreated?: number; errors?: string[]; ticketFilterSummary?: unknown }
      try {
        ...
        try {
          fetch('http://127.0.0.1:7242/ingest/bc0a0876-b22a-43a2-8bb5-3b0f14e7c9c0',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'upload-proto-client.tsx:responseParse',message:'response JSON parse failed',data:{errMsg,responseLength:text.length},timestamp:Date.now(),hypothesisId:'H2'})}).catch(()=>{})
        } catch (_) {}
        // #endregion
        setGenerateResult({ success: false, count: 0, errors: [`Response invalid: ${errMsg}`] })
        return
      }
      ...
      if (!res.ok) {
        ...
        setGenerateResult({
          success: true,
          count: data.signalsCreated || 0,
          errors: data.errors,
          ticketFilterSummary: data.ticketFilterSummary,
        })
      }
    } catch (err) {
      ...
      <main className="container max-w-2xl mx-auto px-4 py-6">
        ...
        {!generateResult && (
          <div className="space-y-4">
                ...
                <div className="space-y-3">
                  {allTabs.map(tab => (
                    <TabQuestionnaire
                      key={tab.key}
                      tab={tab}
                      answers={tabAnswers[tab.key] || { rowType: null, metricColumn: null, dateColumn: null, customRowLabel: null, groupByColumns: [] }}
                      ...
                      onApplyTemplate={(template) => {
                        ...
                        updateTabAnswers(tab.key, {
                          ...
                          customRowLabel: applied.customRowLabel ?? null,
                          groupByColumns: applied.groupByColumns ?? [],
                        })
                      }}
                    />
                  ))}
            ...
            {/* Summary + Generate Button + Save as template */}
            {allSignals.length > 0 && (
                      ...
                      <Dialog open={saveTemplateOpen} onOpenChange={setSaveTemplateOpen}>
                        ...
                        <DialogContent>
                          <DialogHeader>
                            ...
                            <p className="text-sm text-muted-foreground">
                              Reuse this mapping when you upload a file with the same columns.
                            </p>
                          </DialogHeader>
                          <div className="py-2">
                            <label className="text-sm font-medium text-foreground mb-1.5 block">
                          ...
                          <DialogFooter>
                            ...
                            <Button
                              ...
                              onClick={async () => {
                                const tab = completedTabs[0]
                                if (!tab) return
                                const answers = tabAnswers[tab.key]
                                if (!answers?.rowType) return
                                ...
                                try {
                                  const res = await fetch("/api/upload/templates", {
                                    ...
                                    headers: { "Content-Type": "application/json" },
                                    body: JSON.stringify({
                                      displayName: saveTemplateName.trim(),
                                      rowType: answers.rowType,
                                      metricColumn: answers.metricColumn ?? null,
                                      dateColumn: answers.dateColumn ?? null,
                                      customRowLabel: answers.customRowLabel ?? null,
                                      groupByColumns: answers.groupByColumns ?? [],
                                      columns: tab.columns,
                                      ...(organizationId ? { organizationId } : {}),
                                    }),
                                  })
              ...
              <CardContent className="px-4 py-6 text-center">
                {generateResult.success ? (
                  <>
                    ...
                    {generateResult.ticketFilterSummary && generateResult.ticketFilterSummary.some((t) => t.ticketsFiltered > 0) && (
                      <p className="text-xs text-muted-foreground mt-2">
                        Filtered {generateResult.ticketFilterSummary.reduce((s, t) => s + t.ticketsFiltered, 0)} automated ticket{generateResult.ticketFilterSummary.reduce((s, t) => s + t.ticketsFiltered, 0) !== 1 ? "s" : ""} (non-real patterns)
                      </p>
                    )}
                  </>
                ) : (
