...
import { MSS_CATALOG_SIGNAL_NAMES } from "@/lib/mss-catalog"
import { CatalogSignalsView, type CatalogSignalItem } from "@/components/catalog-signals-view"
...
export function SignalsPageClient({ signals: initialSignals, userId, savedSignalIds: initialSavedIds = [], userRole = "manager", preferredKpis = [] }: SignalsPageClientProps) {
  ...
  const [trendDisplay, setTrendDisplay] = useState<TrendDisplay>("percent")
  ...
  const otherSignals = useMemo(
    () => initialSignals.filter((s) => !MSS_CATALOG_SIGNAL_NAMES.includes(s.name)),
    [initialSignals]
  )
  const allSignals = otherSignals
  ...
  const handleRefreshSignals = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        toast({ title: "Refresh failed", description: data?.error ?? "Could not refresh signals", variant: "destructive" })
        return
      }
      toast({ title: "Signals refreshed", description: data.signalsUpdated != null ? `${data.signalsUpdated} signal(s) updated` : "Recalculated from your latest uploads." })
      router.refresh()
    } catch (e) {
  ...
  const hasNoSignals = !catalogLoading && catalogSignals.length === 0 && otherSignals.length === 0

  if (catalogLoading && otherSignals.length === 0) {
    return (
  ...
  if (otherSignals.length > 0 && filteredSignals.length === 0) {
    return (
  ...
  <header className="sticky top-0 z-20 bg-gradient-to-r from-primary to-accent border-b border-border shadow-sm">
    <div className="container max-w-2xl mx-auto px-4 py-3 flex items-center justify-between">
      ...
      <div className="flex items-center gap-1">
        ...
        >
          <RefreshCw className={`h-4 w-4 ${refreshingSignals ? "animate-spin" : ""}`} />
          <span className="sr-only">Refresh signals</span>
        </Button>
        <Link href="/upload">
  ...
  <div className="sticky top-[57px] z-10 bg-card border-b border-border shadow-sm">
        <div className="container max-w-2xl mx-auto px-4 py-3">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="ml-auto shrink-0 flex items-center gap-1 flex-wrap justify-end">
              <span className="text-xs text-muted-foreground mr-1 hidden sm:inline">Trend:</span>
              <Button
                variant={trendPeriod === "30d" ? "secondary" : "ghost"}
                size="sm"
              ...
              <Button
                variant={trendPeriod === "90d" ? "secondary" : "ghost"}
                size="sm"
                className="h-8 px-2 text-xs"
              ...
              <Button
                variant={trendDisplay === "percent" ? "secondary" : "ghost"}
                size="sm"
                className="h-8 px-2 text-xs"
                onClick={() => setTrendDisplay("percent")}
                title="Show trend as percentage"
              >
                %
              </Button>
              <Button
                variant={trendDisplay === "absolute" ? "secondary" : "ghost"}
                size="sm"
                className="h-8 px-2 text-xs"
                onClick={() => setTrendDisplay("absolute")}
                title="Show trend as absolute change"
              >
                Abs
              </Button>
              <Button
      ...
      <main className="container max-w-2xl mx-auto px-4 py-6">
        <CatalogSignalsView
          ...
          orgSignals={initialSignals}
          trendPeriod={trendPeriod}
          trendDisplay={trendDisplay}
          readOnly={false}
          otherSignals={viewMode === "card" ? sortedSignals : []}
          savedSignalIds={savedIds}
          onToggleSave={handleToggleSave}
          savingSignalId={savingSignalId}
          interpretations={interpretations}
          loadingInterpretation={loadingInterpretation}
          onRequestInterpretation={async (signalId) => {
            if (interpretations[signalId] !== undefined || loadingInterpretation === signalId) return
            setLoadingInterpretation(signalId)
            try {
              const res = await fetch(`/api/signals/${signalId}/interpretation`)
              if (res.ok) {
                const data = await res.json()
                setInterpretations((prev) => ({ ...prev, [signalId]: data.interpretation ?? null }))
              } else {
                setInterpretations((prev) => ({ ...prev, [signalId]: null }))
              }
            } catch {
              setInterpretations((prev) => ({ ...prev, [signalId]: null }))
            } finally {
        ...
        {otherSignals.length > 0 && viewMode === "grouped" && (
          <>
            <h2 className="text-sm font-semibold text-muted-foreground flex items-center gap-2 mb-4 mt-8">
