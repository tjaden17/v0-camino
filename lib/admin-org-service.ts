...
/** Remove member by org + user (API path). Use this when members are loaded from Neon details API (member.id is profile id). */
export async function removeMemberFromOrgApi(orgId: string, userId: string) {
  const res = await fetch(`/api/admin/organizations/${encodeURIComponent(orgId)}/members/${encodeURIComponent(userId)}`, {
    method: "DELETE",
  })
  if (!res.ok) {
    const err = await res.json().catch(() => ({}))
...
/** Update user profile in Neon (via API). Use from org details so saved changes appear after refetch. */
export async function updateUserProfileAdminApi(userId: string, updates: Record<string, unknown>) {
